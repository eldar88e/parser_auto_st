module Hamster
  class Seo
    def initialize(domain, name, platform=nil)
      @domain   = domain
      @name     = name
      @platform = platform
    end
    ...
    def indiaps
      title = "#{@name} – Купить на #{@platform} индийский аккаунт"

      desc = "Купите #{@name} на INDIAPS.RU с установкой на индийский аккаунт и сэкономьте до 30% "\
        "по сравнению с турецкими ценами."
      [title, desc]
    end
  end
end
